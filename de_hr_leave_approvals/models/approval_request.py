...
class ApprovalRequest(models.Model):
    ...
    leave_id = fields.Many2one('hr.leave', string='Leave', required=False)
        ...
        for request in self:
            ...
            if status_lst:
                if status_lst.count('cancel'):
                    ...
                    status = 'approved'
                    if request.leave_id:
                        request.leave_id.sudo().action_validate()
                        
                elif status_lst.count('refused') >= minimal_approver:
                    status = 'refused'
                    if request.leave_id:
                        request.leave_id.sudo().action_refuse()           
...
class ApprovalApprover(models.Model):
    ...
    leave_id = fields.Many2one(related='request_id.leave_id')