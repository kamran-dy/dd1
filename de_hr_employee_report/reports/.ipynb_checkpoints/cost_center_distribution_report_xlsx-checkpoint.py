...
class ContractExpiryReport(models.AbstractModel):
    _name = 'report.de_hr_employee_report.cost_center_distribution_xlsx'
    _inherit = 'report.report_xlsx.abstract'
    _description = 'Cost Center Distribution Report XLSX'
    ...
    def generate_xlsx_report(self, workbook, data, lines):
        ...
        sheet.write(3, 0, 'Grade Type', format1)
        sheet.write(3, 1, gr_type, format1)
        ...
        sheet.write(6, 0, 'Emp Code', format1)
        sheet.write(6, 1, 'Name', format1)
        sheet.write(6, 2, 'Grade Type', format1)
        sheet.write(6, 3, 'Employee Type', format1)
        sheet.write(6, 4, 'Cost Center', format1)
        sheet.write(6, 5, '%age Distribution', format1)
        ...
        for o in active_contract:
            ...
            if o.cost_center_information_line[0].percentage_charged:
                percentage_charged = o.cost_center_information_line[0].percentage_charged
            else:
            ...
            sheet.write(row, 0, emp_code, format2)
            sheet.write(row, 1, name, format2)
            sheet.write(row, 2, grade_type, format2)
            sheet.write(row, 3, emp_type, format2)
            sheet.write(row, 4, cost_center, format2)
            sheet.write(row, 5, percentage_charged, format2)
