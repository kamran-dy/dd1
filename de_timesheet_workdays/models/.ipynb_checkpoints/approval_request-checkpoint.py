...
class ApprovalRequest(models.Model):
    ...
    timesheet_att_id = fields.Many2one('hr.timesheet.attendance', string='Timesheet Attendance', required=False)
        ...
        if approver_count == 0:
            ...
            if tot_refused_count ==  tot_approver_count:
                if self.timesheet_att_id:
                    self.timesheet_att_id.sudo().action_refuse()        
        ...
        for request in self:
            ...
            if status_lst:
                if status_lst.count('cancel'):
                    ...
                    status = 'approved'
                    if request.timesheet_att_id:
                        request.timesheet_att_id.sudo().action_approve()
                    
                elif status_lst.count('refused') >= minimal_approver:
                    status = 'refused'
                    if request.timesheet_att_id:
                        request.timesheet_att_id.sudo().action_refuse()    
                else:
                    status = 'pending'
            else:
                status = 'new'
...
class ApprovalApprover(models.Model):
    ...
    timesheet_att_id = fields.Many2one(related='request_id.timesheet_att_id')